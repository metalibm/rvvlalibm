 ...
 *     --target rv64gv --precision single --function log10 --output \
 *     vla_log10_single.c --fname vla_log10_single
 *
 */
...
void vla_log10_single(float* dst, float* src, uint32_t len){
    int32_t remLen;
    int32_t offset;
    ...
    for (;(remLen > INT32_C(0));){
        ...
        rh = vfadd_vv_f32m1(tmp49, tmp64, l);
        tmp65 = vfmul_vf_f32m1(rh, 0x1.bcb7b2p-2f, l);
        tmp66 = vfsub_vv_f32m1(rh, tmp49, l);
        rl = vfsub_vv_f32m1(tmp64, tmp66, l);
        tmp67 = vfmsub_vf_f32m1(rh, 0x1.bcb7b2p-2f, tmp65, l);
        tmp68 = vfmadd_vf_f32m1(rh, -0x1.5b235ep-27f, tmp67, l);
        tmp69 = vfmadd_vf_f32m1(rl, 0x1.bcb7b2p-2f, tmp68, l);
        result = vfadd_vv_f32m1(tmp65, tmp69, l);
        tmp70 = dst + offset;
        vse32_v_f32m1(tmp70, result, l);
        tmp71 =  (int32_t) l;
        tmp72 = remLen - tmp71;
        remLen = tmp72;
        tmp73 = offset + tmp71;

 ...
 *     --target rv64gv --precision single --function log2 --output \
 *     vla_log2_single.c --fname vla_log2_single
 *
 */
...
void vla_log2_single(float* dst, float* src, uint32_t len){
    int32_t remLen;
    int32_t offset;
    ...
    for (;(remLen > INT32_C(0));){
        ...
        tmp65 = vfmul_vf_f32m1(rh, 0x1.715476p0f, l);
        tmp66 = vfsub_vv_f32m1(rh, tmp49, l);
        rl = vfsub_vv_f32m1(tmp64, tmp66, l);
        tmp67 = vfmsub_vf_f32m1(rh, 0x1.715476p0f, tmp65, l);
        tmp68 = vfmadd_vf_f32m1(rh, 0x1.4ae0cp-26f, tmp67, l);
        tmp69 = vfmadd_vf_f32m1(rl, 0x1.715476p0f, tmp68, l);
